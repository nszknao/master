...
//     	syslog(LOG_ERR, "addBasePath :".$path);
...
//        $this->addTemplateDir($path . 'static');
    ...
    private function _loadSmarty()
    {
		...
		if(isset($this->_config['caching'])) $this->_smarty->caching = $this->_config['caching'];
        if(isset($this->_config['compile_check'])) $this->_smarty->compile_check = $this->_config['compile_check'];
        if(isset($this->_config['cache_lifetime'])) $this->_smarty->cache_lifetime = $this->_config['cache_lifetime'];
        if(isset($this->_config['template_dir'])) $this->_smarty->template_dir = $this->_config['template_dir'];
        if(isset($this->_config['compile_dir'])) $this->_smarty->compile_dir = $this->_config['compile_dir'];
        if(isset($this->_config['config_dir'])) $this->_smarty->config_dir = $this->_config['config_dir'];
        if(isset($this->_config['cache_dir'])) $this->_smarty->cache_dir = $this->_config['cache_dir'];
        if(isset($this->_config['left_delimiter'])) $this->_smarty->left_delimiter = $this->_config['left_delimiter'];
        if(isset($this->_config['right_delimiter'])) $this->_smarty->right_delimiter = $this->_config['right_delimiter'];
        if(isset($this->_config['force_compile'])) $this->_smarty->force_compile = $this->_config['force_compile'];
//        $this->assign('this', $this);
//		$this->_smarty->loadFilter("variable", "htmlspecialchars");
    }
    /**
     *
     * テンプレートをFetchしてBufferに出力する。
     * @return string
     */
    public function fetch() {
    	return $this->_smarty->fetch($this->_customTemplate);
    }
}